...
import { processExcelFileClientesSegregados } from "@/features/mapa/utils/clientesSegregados"
...
export function UploadClientesSegregados({ onUploadComplete }: UploadClientesSegregadosProps) {
  ...
  const { setClientesSegregar } = useConfigImpressaoStore()
  ...
  const handleUpload = async () => {
    ...
    try {
      const clientes = await processExcelFileClientesSegregados(uploadedFile)
      const codigosClientes = clientes.map(cliente => cliente.codCliente)
      
      // Atualiza o store com os novos clientes
      setClientesSegregar(codigosClientes)
